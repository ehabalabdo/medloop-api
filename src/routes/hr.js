...
// PDF route BEFORE global auth â€” handles its own token via query param
router.get("/payslips/:id/pdf", async (req, res) => {
  try {
    ...
    const infoY = doc.y;
    doc.text(`Name: ${decrypt(p.employee_name)}`, 50, infoY);
    ...
    logger.error("GET /hr/payslips/:id/pdf error:", err);
...
async function consumeBioToken(req, employeeId, bioToken) {
  ...
  if (row.issue_ip && reqIp && row.issue_ip !== reqIp) {
    ...
    if (!isLoopback) {
      logger.warn("[bioToken] IP mismatch â€” issue:", row.issue_ip, "use:", reqIp, "employee:", employeeId);
...
// ============================================================
//  DEBUG: WebAuthn diagnostic â€” DEV/STAGING ONLY
// ============================================================
...
router.patch("/clinic/location", async (req, res) => {
  ...
  try {
    ...
    logger.error("PATCH /hr/clinic/location error:", err);
...
router.get("/clinic/location", async (req, res) => {
  try {
    ...
    logger.error("GET /hr/clinic/location error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/employees", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/employees error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/employees", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/employees error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.put("/employees/:id", async (req, res) => {
  ...
  try {
    ...
    // Update employee row. Encrypt PII if present; blind-index updates
    // happen separately so the unique constraint on email_idx still works.
    const { rows } = await pool.query(
      `UPDATE hr_employees SET
         ...
         email_idx=COALESCE($9, email_idx),
         phone_idx=COALESCE($10, phone_idx),
         updated_at=NOW()
       WHERE id=$7 AND client_id=$8 RETURNING *`,
      [
        full_name != null ? encrypt(full_name) : null,
        phone != null ? encrypt(phone) : null,
        email != null ? encrypt(email) : null,
        status, basic_salary, role, id, client_id,
        email != null ? blindIndex(email) : null,
        phone != null ? blindIndex(String(phone).replace(/\D/g, "")) : null,
      ]
    );
    ...
    logger.error("PUT /hr/employees/:id error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.delete("/employees/:id", async (req, res) => {
  ...
  try {
    ...
    logger.error("DELETE /hr/employees/:id error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/employees/:id/reset-password", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/employees/:id/reset-password error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/me", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/me error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/me/set-pin", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/me/set-pin error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/webauthn/register/options", async (req, res) => {
  ...
  try {
    ...
    const options = await generateRegistrationOptions({
      ...
      userName: e.username,
      userDisplayName: decrypt(e.full_name),
    ...
    logger.info("[WebAuthn Reg Options] rpID:", rpID, "employee:", e.username, "challenge:", options.challenge?.slice(0, 10) + "...");
    ...
    logger.error("POST /hr/webauthn/register/options error:", err);
...
router.post("/webauthn/register/verify", async (req, res) => {
  ...
  try {
    ...
    logger.info("[WebAuthn Reg Verify] verified:", verification.verified, "rpID:", rpID, "origin:", origin);

    if (!verification.verified || !verification.registrationInfo) {
      logger.error("[WebAuthn Reg Verify] FAILED â€” verified:", verification.verified);
    ...
    logger.info("[WebAuthn Reg Verify] credentialId:", credIdB64.slice(0, 20) + "...", "type:", credentialDeviceType, "idType:", typeof credential.id);
    ...
    logger.info("[WebAuthn Reg Verify] SUCCESS â€” credential saved for employee:", hr_employee_id);
    res.json({ verified: true });
  } catch (err) {
    logger.error("POST /hr/webauthn/register/verify error:", err);
    res.status(500).json({ error: "Server error: " + (err?.message || String(err)) });
  }
});
...
router.delete("/webauthn/reset", async (req, res) => {
  ...
  try {
    ...
    logger.info("[WebAuthn Reset] Cleared", del.rowCount, "credentials for employee:", hr_employee_id);
    res.json({ cleared: del.rowCount });
  } catch (err) {
    logger.error("DELETE /hr/webauthn/reset error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/webauthn/authenticate/options", async (req, res) => {
  ...
  try {
    ...
    logger.info("[WebAuthn Auth Options] rpID:", rpID, "creds:", creds.rows.length, "employee:", hr_employee_id);
    ...
    logger.error("POST /hr/webauthn/authenticate/options error:", err);
    res.status(500).json({ error: "Server error: " + (err?.message || String(err)) });
  }
});
...
router.post("/webauthn/authenticate/verify", async (req, res) => {
  ...
  try {
    ...
    logger.info("[WebAuthn Auth Verify] Looking for credential:", credIdFromBody?.slice(0, 30));
    ...
    if (!cred.rows.length) {
      ...
      logger.error("[WebAuthn Auth Verify] Credential NOT FOUND. Body id:", credIdFromBody);
      logger.error("[WebAuthn Auth Verify] Stored credentials:", allCreds.rows.map(r => r.credential_id));
      return res.status(400).json({ error: "Credential not found", code: "CRED_NOT_FOUND" });
    }

    const storedCred = cred.rows[0];
    logger.info("[WebAuthn Auth Verify] Found credential id:", storedCred.id, "counter:", storedCred.counter);
    ...
    try {
      ...
      logger.error("[WebAuthn Auth Verify] VERIFY THREW ERROR:", verifyErr.message);
      logger.error("[WebAuthn Auth Verify] rpID:", rpID, "origin:", origin);
      return res.status(400).json({ error: "Biometric verification failed" });
    }

    logger.info("[WebAuthn Auth Verify] verified:", verification.verified);
    ...
    // from a different network. (issue_* columns are added by migration 002 â€”
    ...
    logger.error("POST /hr/webauthn/authenticate/verify error:", err);
    res.status(500).json({ error: "Server error: " + (err?.message || String(err)) });
  }
});
...
router.post("/attendance/check-in", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/attendance/check-in error:", err);
    res.status(500).json({ error: "Server error: " + (err?.message || String(err)) });
  }
});
...
router.post("/attendance/check-out", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/attendance/check-out error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
  ...
  try {
    ...
    res.json(
      rows.map((r) => ({
        ...
        employeeName: decrypt(r.employee_name),
    ...
    logger.error("GET /hr/attendance error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/reports/monthly", async (req, res) => {
  try {
    ...
    logger.error("GET /hr/reports/monthly error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/reports/my-monthly", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/reports/my-monthly error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/attendance/break-out", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/attendance/break-out error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/attendance/break-in", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/attendance/break-in error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/attendance/:employeeId/timeline", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/attendance/:id/timeline error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/deductions", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/deductions error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/deductions", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/deductions error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.delete("/deductions/:id", async (req, res) => {
  ...
  try {
    ...
    logger.error("DELETE /hr/deductions/:id error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/warnings", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/warnings error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/warnings", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/warnings error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/notifications", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/notifications error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/notifications", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/notifications error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/notifications/my", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/notifications/my error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.patch("/notifications/:id/read", async (req, res) => {
  ...
  try {
    ...
    logger.error("PATCH /hr/notifications/:id/read error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/social-security", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/social-security error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.patch("/social-security", async (req, res) => {
  ...
  try {
    ...
    logger.error("PATCH /hr/social-security error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/payroll/generate", async (req, res) => {
  ...
  try {
    ...
    for (const emp of employees.rows) {
      ...
      payslips.push({ employeeId: emp.id, employeeName: decrypt(emp.full_name), netSalary });
    }

    res.json({ runId, month, status: 'draft', totalEmployees: payslips.length, payslips });
  } catch (err) {
    logger.error("POST /hr/payroll/generate error:", err);
    res.status(500).json({ error: "Server error: " + (err?.message || String(err)) });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      payslips: payslips.rows.map(p => ({
        ...
        employeeName: decrypt(p.employee_name),
    ...
    logger.error("GET /hr/payroll/run error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/payslips/:id", async (req, res) => {
  ...
  try {
    ...
    logger.error("GET /hr/payslips/:id error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.patch("/payslips/:id", async (req, res) => {
  ...
  try {
    ...
    logger.error("PATCH /hr/payslips/:id error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/payslips/:id/approve", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/payslips/:id/approve error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/payslips/:id/reject", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/payslips/:id/reject error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
...
router.post("/payroll/close-month", async (req, res) => {
  ...
  try {
    ...
    logger.error("POST /hr/payroll/close-month error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
